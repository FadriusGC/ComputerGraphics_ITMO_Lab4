...
void BoxApp::Update(const GameTimer& gt) {
  ...
  composeConstants.CascadeSplits = mRenderingSystem.GetCascadeSplits();
  const auto& shadowViewProjMatrices =
      mRenderingSystem.GetShadowViewProjMatrices();
  ...
  composeConstants.ShadowParams = DirectX::SimpleMath::Vector4(
      ...
      static_cast<float>(mRenderingSystem.GetShadowCascadeCount()),
      static_cast<float>(mRenderingSystem.GetShadowsEnabled()), 0.0f);
...
void BoxApp::Draw(const GameTimer& gt) {
  ...
  mRenderingSystem.Render(
      ...
      gt.DeltaTime(), mView, mProj, mView * mProj, mCamPos);
