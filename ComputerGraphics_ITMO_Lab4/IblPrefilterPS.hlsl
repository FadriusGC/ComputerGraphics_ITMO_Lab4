// Self-contained (no #include). GGX prefilter (lecture slides 65-66).
...
float4 PS(VSOut input) : SV_Target {
    ...
    [loop] for (uint i = 0u; i < SAMPLE_COUNT; ++i) {
        ...
        float NdotL = max(dot(N, L), 0.0f);
        if (NdotL > 0.0f) {
            float3 c = gEnvMap.SampleLevel(gSampler, L, 0).rgb;
            c = min(c, 60.0f);  // clamp to tame fireflies from the bright sun
